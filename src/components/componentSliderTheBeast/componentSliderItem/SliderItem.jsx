...
//   // Проверяем, что size - массив (размеры), иначе, если строка, проверяем "универсальный"
//   const isUniversal = typeof product.size === 'string' && product.size.toLowerCase().includes('универс')

//   // Если размеры - массив, формируем список размеров, иначе пустой массив (будет использоваться для рендера размеров)
...
//     if (isUniversal) {
//       // Для универсального размера - можно сразу добавить
//       dispatch(addToCart({ ...product, selectedSize: 'универсальный'}))
//       setSizeError('')
//     } else if (!selectedSize) {
//       // Для обычных размеров - если не выбран размер, показываем ошибку
//       setSizeError('Выберите размер')
//     } else {
//       // Размер выбран - добавляем в корзину
...
//           <span className="sliderItemNameSneakers__nameSneakers">{product.name}<br />{product.brand}</span>
...
//         >
//           <FontAwesomeIcon
//             className="sliderItemPriceSneakers__svgLike like"
//             icon={faHeart}
//           />
//         </div>
//       </div>

//       {/* Рендерим размеры только если это не универсальный размер */}
//       {!isUniversal && (
